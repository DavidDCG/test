...
const CasetasRutas = async (req = request, res = response) => {
  ...
  try {
   ...
   let validaCasetas = validarJSONCasetasRutas(jsonDatosCalculo);
    ...
    if (tipoCalculo == 2) {
      ...
      const requestOptions = {
        ...
        if (body.status == "OK")
        {
          ...
          res.json({"error": messageerror + " KEY: " + process.env.APLICATION_KEY_TOLLGURU + ' url : '+ process.env.API_URL_ROOT_TOLLGURU  });
        }
    ...
    res.json({"error": e.message + " KEY: " + process.env.APLICATION_KEY_TOLLGURU + ' url : '+ process.env.API_URL_ROOT_TOLLGURU  });
